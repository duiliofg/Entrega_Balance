...
% Link meteo
link_meteo=strcat('/directorio/Forcings_Glaciers_v25');
%Guardar archivos
model='/V25_TcDEM_hnmean_1981_2021_fix_nodatagl';
main='/directorio/resultados';
...
S=shaperead("/directorio/inventario_glaciar");
...
for i=l_inicio:l_final
    ...
    if exist(strcat(link_meteo,'/data_',num2str(lat(fila_2000)),'_',num2str(lon(col))),'file') > 0 || exist(strcat(link_meteo,'/data_CL',num2str(info(1)),'.txt'),'file')>0 %Dato
        ...
        if info(:,7)/10^6>=25
            ...
            t2m=file_temp_poligonal(:,7);
            t2m_c=file_temp_poligonal(:,7)+Tc;
            ...
            elseif exist('temperatura')==false
                ...
                t2m=file_temp_puntual(:,6);
                t2m_c=file_temp_puntual(:,6)+Tc;
            ...
            t2m_c=average_timeseries(t2m_c,8);
    ...
    if std_data_for_map(index_db,2)==1  %if zonal
        ...
        tc=0;
        pairs_zn=[c1,c0,c2];
        vapourpressuresaturation=(6.116441*10.^((7.591386.*t2m_c)./(t2m_c+240.7263))).*100;
        ...
        x_diario=[tiempo_day pr t2m_c rh pa ws vapourpressure sw lw]; % Versión acotada
...
%% Cracion de tablas 
   ...
   if std_data_for_map(i,2)==1
                    Cabeceras_diarias = {'Año';'Mes';'Día';'SW [W/m^2]';'Temperatura [°C]';'Temperatura corregida[°C]';'Precipitación [mm]';'Precipitación sólida[mm]';'Área[km2]';'Volumen[km3]';'SWE [mm]';'Derretimiento nieve[mm]';'Derretimiento hielo [mm]';'Caudal Nival[m3/s]';'Caudal Hielo[m3/s]';'Caudal Total[m3/s]';'Sublimación [mm]';'dh/dt [m/yr]'};
                    tabla_diaria=table(yr,mth,day,sw,t2m,t2m_c,pr,flujos_gl(:,13),flujos_gl(:,2),flujos_gl(:,3),flujos_gl(:,8),flujos_gl(:,9),flujos_gl(:,7),flujos_gl(:,14),flujos_gl(:,12),flujos_gl(:,15),flujos_gl(:,end),dhdt,'VariableNames',Cabeceras_diarias);
   ...
   elseif std_data_for_map(i,2)==2
                    Cabeceras_diarias = {'Año';'Mes';'Día';'SW [W/m^2]';'Temperatura [°C]';'Temperatura corregida[°C]';'Precipitación [mm]';'Precipitación sólida[mm]';'Área[km2]';'Volumen[km3]';'SWE [mm]';'Derretimiento nieve[mm]';'Derretimiento hielo [mm]';'Caudal Nival[m3/s]';'Caudal Hielo[m3/s]';'Caudal Total[m3/s]';'Sublimación [mm]';'dh/dt [m/yr]'};
                    tabla_diaria=table(yr,mth,day,sw,t2m,t2m_c,pr,flujos_gl(:,13),flujos_gl(:,2),flujos_gl(:,3),flujos_gl(:,8),flujos_gl(:,9),flujos_gl(:,7),flujos_gl(:,14),flujos_gl(:,12),flujos_gl(:,15),nan(length(yr),1),dhdt,'VariableNames',Cabeceras_diarias);
